import { useGetStarshipByIdQuery } from '../../features/apiSlice'
import { DetailRow, Spinner, Error } from '../../components'

export const SingleStarshipDetail = ({ id }) => {
  const { data, isLoading, isError } = useGetStarshipByIdQuery(id)
    ...
    <article className="flex flex-col md:flex-row gap-4 p-6 shadow-lg overflow-hidden">
      ...
      <div className="w-full md:w-2/3 flex flex-col px-6">
        ...
        <div className="flex flex-col gap-2 ">
          <DetailRow label={'Models'} value={data.model} />
          ...
          <DetailRow label={'Hyperdrive rating'} value={data.hyperdrive_rating} />
          <DetailRow label={'MGLT'} value={data.MGLT} />
          <DetailRow label={'Starship class'} value={data.starship_class} />
